...
=end
  def initialize( xbee_usbdev_str, baud, data_bits, stop_bits, parity )
    ...
    @xbee = SerialPort.new( xbee_usbdev_str, baud.to_i, data_bits.to_i, stop_bits.to_i, parity )
    @xbee.read_timeout = TYPICAL_READ_TIMEOUT
...
=end rdoc 
  ...
  def neighbors
    ...
    tmp = @xbee.read_timeout 
    @xbee.read_timeout = LONG_READ_TIMEOUT
    @xbee.write("ATND\r")
    response = getresponse  
...
=end
 ...
 def baud!( baud_rate )  
    @xbee.write("ATBD#{@baudcodes[baud_rate]}")
    getresponse 
 end
...
=end
 ...
 def parity!( parity_type )
   ...
   @xbee.write("ATNB#{@paritycodes[parity_type]}")
   getresponse 
 end
...
=end
  ...
  def dio!( port, iotype )
    at = "AT#{port.to_s}#{@iotypes[iotype]}"
    @xbee.write( at )
    getresponse
  end
